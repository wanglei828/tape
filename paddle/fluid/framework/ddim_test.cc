...
TEST(DDim, Equality) {
  ...
  ddim[1] = 2;
  EXPECT_EQ(ddim[1], 2);
  paddle::fluid::framework::set(ddim, 0, 6);
  EXPECT_EQ(paddle::fluid::framework::get(ddim, 0), 6);

  // vectorize a DDim
  std::vector<int64_t> res_vec = paddle::fluid::framework::vectorize(vddim);
  ...
  // add two DDims
  paddle::fluid::framework::DDim ddim_sum = ddim + vddim;
  EXPECT_EQ(ddim_sum[0], 15);
  EXPECT_EQ(ddim_sum[1], 3);
  EXPECT_EQ(ddim_sum[2], 10);

  // multiply two DDims
  paddle::fluid::framework::DDim ddim_mul = ddim * vddim;
  EXPECT_EQ(ddim_mul[0], 54);
  EXPECT_EQ(ddim_mul[1], 2);
  EXPECT_EQ(ddim_mul[2], 25);

  // arity of a DDim
  EXPECT_EQ(paddle::fluid::framework::arity(ddim), 3);

...
namespace framework = paddle::fluid::framework;
namespace platform = paddle::fluid::platform;
...
TEST(Tensor, Layout) {
  framework::Tensor src;
  ASSERT_EQ(src.layout(), framework::TensorDataLayout::kNCHW);
  src.set_layout(framework::TensorDataLayout::kAnyLayout);
  ASSERT_EQ(src.layout(), framework::TensorDataLayout::kAnyLayout);
}

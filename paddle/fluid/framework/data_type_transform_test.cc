...
TEST(DataTypeTransform, CPUTransform) {
  ...
  // data type transform from/to float16
  {
    ...
    paddle::fluid::framework::TransDataType(kernel_fp16, kernel_fp32, in, &out);
    float* out_data_float = out.data<float>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(out_data_float[i], static_cast<float>(ptr[i]));
    }

    paddle::fluid::framework::TransDataType(kernel_fp16, kernel_fp64, in, &out);
    double* out_data_double = out.data<double>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(out_data_double[i], static_cast<double>(ptr[i]));
    }

    paddle::fluid::framework::TransDataType(kernel_fp16, kernel_int32, in, &out);
    int* out_data_int = out.data<int>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(out_data_int[i], static_cast<int>(ptr[i]));
    }

    paddle::fluid::framework::TransDataType(kernel_fp16, kernel_int64, in, &out);
    int64_t* out_data_int64 = out.data<int64_t>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(out_data_int64[i], static_cast<int64_t>(ptr[i]));
    }

    paddle::fluid::framework::TransDataType(kernel_fp16, kernel_bool, in, &out);
    ...
    paddle::fluid::framework::TransDataType(kernel_fp32, kernel_fp16, in, &out);
    ...
    double* in_data_double =
        in.mutable_data<double>(paddle::fluid::framework::make_ddim({2, 3}), place);
    for (int i = 0; i < data_number; ++i) {
      in_data_double[i] = i;
    }

    paddle::fluid::framework::TransDataType(kernel_fp64, kernel_fp16, in, &out);
    ptr = out.data<paddle::fluid::platform::float16>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(ptr[i].x,
    ...
    int* in_data_int =
        in.mutable_data<int>(paddle::fluid::framework::make_ddim({2, 3}), place);
    for (int i = 0; i < data_number; ++i) {
      in_data_int[i] = i;
    }

    paddle::fluid::framework::TransDataType(kernel_int32, kernel_fp16, in, &out);
    ptr = out.data<paddle::fluid::platform::float16>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(ptr[i].x,
    ...
    int64_t* in_data_int64 =
        in.mutable_data<int64_t>(paddle::fluid::framework::make_ddim({2, 3}), place);
    for (int i = 0; i < data_number; ++i) {
      in_data_int64[i] = i;
    }

    paddle::fluid::framework::TransDataType(kernel_int64, kernel_fp16, in, &out);
    ptr = out.data<paddle::fluid::platform::float16>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(ptr[i].x,
    ...
    bool* in_data_bool =
        in.mutable_data<bool>(paddle::fluid::framework::make_ddim({2, 3}), place);
    for (int i = 0; i < data_number; ++i) {
      in_data_bool[i] = i;
    }

    paddle::fluid::framework::TransDataType(kernel_bool, kernel_fp16, in, &out);
    ptr = out.data<paddle::fluid::platform::float16>();
    for (int i = 0; i < data_number; ++i) {
      EXPECT_EQ(ptr[i].x,
